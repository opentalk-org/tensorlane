...
async fn data_cache_populates_removes_consumed_files_and_cleans_up_per_run() -> Result<()> {
    ...
    let config = data_config(dataset, 2);
    ...
    let before = eventually("two prefetched training batches", || async {
    ...
    assert_eq!(response[0].batch.len(), 1);
    ...
    let consumed = before
        .difference(&after)
        .next()
        .context("no cache file removed")?;
    assert_eq!(response[0].batch[0].wave.as_ref(), original[consumed]);
...
async fn finish_cancels_blocked_prefetch_and_removes_cache() -> Result<()> {
    let env = TestEnv::start().await?;
    let dataset = env.seed_dataset(2).await?;
    let run = env.create_run(data_config(dataset, 20), json!({})).await?;
    env.init_run(&run).await?;
    eventually("training cache filled", || async {
        Ok((files(&env.run_cache(&run).join("data/training"))?.len() == 5).then_some(()))
    })
    .await?;
    env.pause_s3().await?;
